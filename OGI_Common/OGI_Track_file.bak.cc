...
size_t OGIfwrite_track(EST_Track &tr, FILE *fp, bool compress){
  int i;
  char chnum[100];
  size_t fps = ftell(fp);
...
static void write_compressed_track_data(EST_Track &tr, FILE *fp){
  ...
  for (j = 0; j < tr.num_channels(); ++j){
    ...
    if (maxCh-minCh < 10e-10)      maxCh = minCh+10e-10;
...
size_t OGIfwrite_track(OGI_Track &tr, FILE *fp, bool compress){
  int i;
  char chnum[100];
  size_t fps = ftell(fp);
