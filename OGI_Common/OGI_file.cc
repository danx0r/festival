...
// no swap needed
int OGIfread(char *ptr, int sz, int N, FILE *fp, bool swap=0){
  swap = 0;
  fread_check(ptr, sz, N, fp);
  return N;
}
int OGIfread(unsigned char *ptr, int sz, int N, FILE *fp, bool swap=0){
  swap = 0;
  fread_check(ptr, sz, N, fp);
  return N;
}
// don't need as long as OGIbool==char
//int OGIfread(OGIbool *ptr, int sz, int N, FILE *fp, bool swap=0){
//  swap = 0;
//  fread_check(ptr, sz, N, fp);
//  return N;
//}
...
size_t OGIfread(EST_TVector<OGIbool> &bv, int N, FILE *fp, bool swap){
...
size_t OGIfread(EST_IVector &iv, int N, FILE *fp, bool swap){
  int* tmp = new int [N];
  if (tmp == NULL) cerr << "OGIfread - insufficient memory" << endl,exit(-1);
...
size_t OGIfread(EST_FVector &fv, int N, FILE *fp, bool swap){
...
// swap needed
int OGIfread(int *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
    swap_bytes_int(ptr, N);
  return N;
}
int OGIfread(short *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
    swap_bytes_short(ptr, N);
  return N;
}
int OGIfread(unsigned short *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
    swap_bytes_ushort(ptr, N);
  return N;
}
int OGIfread(long *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
    swap_bytes_long(ptr, N);
  return N;
}
int OGIfread(float *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
    swap_bytes_float(ptr, N);
  return N;
}
int OGIfread(double *ptr, int sz, int N, FILE *fp, bool swap){
  fread_check(ptr, sz, N, fp);
  if (swap)
